...
func promptForAddress(r io.Reader) (*api.Person, error) {
	p := &api.Person{}
	...
	for {
		...
		// the nested message.  Just as with api.Person, it can be
		// created like any other struct.
		pn := &api.Person_PhoneNumber{
			Number: phone,
		}
		...
		switch ptype {
		case "mobile":
			pn.Type = api.Person_MOBILE
		case "home":
			pn.Type = api.Person_HOME
		case "work":
			pn.Type = api.Person_WORK
		default:
